...
const Source: FC = () => {
  ...
  const activeGroup = useMemo(
    () => groups.find((x) => x.uniqueKey === activeGroupKey),
    ...
    (record) => {
      Modal.confirm({
        ...
        onOk() {
          const newGroups = cloneDeep(groups)
          const groupIndex = newGroups.findIndex((x) => x.uniqueKey === activeGroupKey)
          newGroups[groupIndex].children = newGroups[groupIndex].children.filter(
      ...
      <ItemModal
        ...
        onOk={(newItem) => {
          ...
          if (itemModalRef.current.mode === 'add') {
            ...
            newGroups[groupIndex].children = [...newGroups[groupIndex].children, newItem]
          } else {
            const tableIndex = newGroups[groupIndex].children.findIndex(
              (x) => x.uniqueKey === newItem.uniqueKey,
            )
