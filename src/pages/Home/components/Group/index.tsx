...
const Group: FC<any> = ({ onEdit, onDelete, onAdd, value, activeKey, onItemClick, onItemCheck }) => {
  ...
  if (!(value && value.length)) {
    return (
      <Empty
        ...
        description={<span>当前无分组，请先创建</span>}
      >
      ...
      {value.map((item) => {
        return (
          <Menu.Item key={item.uniqueKey} className="group-container-item">
            ...
            />
            <span className="group-container-item-label" title={item.name}>
              {item.name}
            </span>
            <Dropdown
              overlay={
            ...
            >
              <FormOutlined className="group-container-item-operator" />
            </Dropdown>
          </Menu.Item>
        )
      })}
